...
const runningUtils = {
  ...
  countKmsAndTime: (runningData) => {
    return {
      ...
      Temps: globalUtils.formatDuration(
        runningData.sessions.reduce((accSes, session) => {
          return (
            accSes +
            session.exercises.reduce((accEx, exercise) => {
              return accEx + runningUtils.sumReducerTimes(exercise.kmsTimes, exercise.kms);
            }, 0)
          );
        }, 0)
      ),
  ...
  getAllure: (runningExercise) => runningUtils.sumReducerTimes(runningExercise.kmsTimes, runningExercise.kms) / runningExercise.kms,
};
